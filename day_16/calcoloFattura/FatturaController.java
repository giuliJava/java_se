	...
	public static void calcoloFattura(Cliente cliente) {
		...
		if (cliente instanceof ClienteLuce) {
			...
			if (clienteFisso.getContratto() == ContrattoFisso.STANDARD) {
				clienteFisso.setPrezzoDaPagare(BigDecimal.valueOf(30.0));
			} else if (clienteFisso.getContratto() == ContrattoFisso.SPECIAL) {
				clienteFisso.setPrezzoDaPagare(BigDecimal.valueOf(50.0));
			} else {
				clienteFisso.setPrezzoDaPagare(BigDecimal.valueOf(60.0));
			}
		} else {
			ClienteMobile clienteMobile = (ClienteMobile) cliente;
			if (clienteMobile.getGiga() <= 30) {
				clienteMobile.setPrezzoDaPagare(BigDecimal.valueOf(8.0));
			} else if (clienteMobile.getGiga() > 30 && clienteMobile.getGiga() < 80) {
				clienteMobile.setPrezzoDaPagare(BigDecimal.valueOf(12.0));
			} else {
				clienteMobile.setPrezzoDaPagare(BigDecimal.valueOf(15.0));
			}
		}
	}
}
